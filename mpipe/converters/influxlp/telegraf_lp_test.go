...
func readNextToken(lp string, begin int) (string, int) {
...
func parseLP(lp string) {
	...
	measure, cur := readNextToken(lp, 0)
	fmt.Println(measure)

	for lp[cur] != ' ' {
		var tagName, tagValue string
		tagName, cur = readNextToken(lp, cur+1)
		tagValue, cur = readNextToken(lp, cur+1)
	...
	for lp[cur] != ' ' {
		...
		fieldName, cur = readNextToken(lp, cur+1)
		fieldValue, cur = readNextToken(lp, cur+1)
